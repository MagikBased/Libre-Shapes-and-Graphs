...
func _ready() -> void:
	...
	derivative_plot = GShapes.DerivativePlot2D.new()
	...
	x_tracker = GShapes.ValueTracker.new(-3.4)
	add_child(x_tracker)
	...
	play_group([
		...
		GShapes.ShowCreation.new(derivative_plot, 1.2, &"smooth"),
		GShapes.FadeIn.new(marker, 0.4, &"smooth"),
	])
	play_sequence([
		GShapes.SetValue.new(x_tracker, 6.3, 1.6, &"smooth"),
		GShapes.SetValue.new(x_tracker, -1.0, 1.2, &"smooth"),
		GShapes.SetValue.new(x_tracker, 4.2, 1.0, &"smooth"),
	])
