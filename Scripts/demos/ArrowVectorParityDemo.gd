...
func _ready() -> void:
	...
	vector_arrow = GShapes.Arrow2D.new()
	...
	magnitude_readout = GShapes.DecimalNumber.new(0.0, 2, false, "")
	...
	play(GShapes.ShowCreation.new(vector_arrow, 1.1, &"smooth"))
	wait(0.2)
	play_sequence([
		GShapes.SetValue.new(phase_tracker, TAU * 1.0, 1.6, &"smooth"),
		GShapes.SetValue.new(phase_tracker, TAU * 2.2, 1.8, &"linear"),
		GShapes.SetValue.new(phase_tracker, TAU * 3.1, 1.7, &"smooth"),
	])
