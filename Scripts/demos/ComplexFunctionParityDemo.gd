...
var base_path: GShapesComplexFunctionPath2D
var mapped_path: GShapesComplexFunctionPath2D
...
func _ready() -> void:
	...
	base_path = GShapes.ComplexFunctionPath2D.new()
	...
	mapped_path = GShapes.ComplexFunctionPath2D.new()
	...
	play(GShapes.FadeIn.new(base_path, 0.4, &"smooth"))
	play(GShapes.ShowCreation.new(mapped_path, 0.8, &"smooth"))
	wait(0.15)
	play_sequence([
		GShapes.SetValue.new(tracker, 0.6, 1.25, &"smooth"),
		GShapes.SetValue.new(tracker, 1.0, 1.15, &"smooth"),
		GShapes.SetValue.new(tracker, 0.35, 1.0, &"linear"),
	])
