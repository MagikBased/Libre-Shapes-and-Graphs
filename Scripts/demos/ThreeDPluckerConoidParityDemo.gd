# Demo: ThreeDPluckerConoidParityDemo
# Expected behavior: See PlansAndCopy/DEMO_NOTES.md

extends GShapes3DScene
var axes: GShapesAxes3D
var conoid: GShapesPluckerConoid3D
...
func _ready() -> void:
	...
	conoid = GShapes.PluckerConoid3D.new()
	conoid.mode_name = modes[mode_index]
	conoid.u_steps = 108
	conoid.v_steps = 72
	conoid.scale_factor = 1.1
	conoid.surface_color = colors[mode_index]
	add_child(conoid)
...
func _process(delta: float) -> void:
	...
	conoid.phase = time_accum
	conoid.rotation.y += delta * 0.2
...
func _create_overlay() -> void:
	...
	add_child(layer)
	var label := Label.new()
	label.text = "3D plucker-conoid parity: GShapesPluckerConoid3D | Space/1/2/3 mode, RMB orbit, MMB pan, wheel zoom, R reset, F reframe"
