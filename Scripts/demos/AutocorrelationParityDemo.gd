...
var corr_plot: GShapesAutocorrelationPlot2D
...
func _ready() -> void:
	...
	corr_plot = GShapes.AutocorrelationPlot2D.new()
	corr_plot.axes = axes
	...
	play(GShapes.FadeIn.new(corr_plot, 0.5, &"smooth"))
	play_sequence([
		...
		GShapes.SetValue.new(parameter_tracker, 3.79, 1.0, &"smooth"),
	])
