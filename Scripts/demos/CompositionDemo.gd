...
func _ready() -> void:
	...
	play_group([
		GShapes.MoveTo.new(circle_a, Vector2(520.0, 240.0), 1.1, &"smooth"),
		GShapes.FadeToColor.new(circle_a, Color.ORANGE_RED, Color.GOLD, 1.1, &"smooth"),
	])
	wait_seconds(0.3)
	play_sequence([
		GShapes.MoveTo.new(circle_b, Vector2(520.0, 440.0), 0.9, &"smooth"),
		GShapes.Transform.new(circle_b, null, Vector2(1.6, 1.6), deg_to_rad(120.0), 0.9, &"smooth"),
		GShapes.MoveTo.new(circle_b, Vector2(980.0, 440.0), 0.9, &"smooth"),
	])
