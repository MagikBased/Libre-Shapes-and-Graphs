...
func _ready() -> void:
	...
	play(GShapes.FadeIn.new(marker, 0.25, &"smooth"))
	wait(0.2)
	play_sequence([
		GShapes.SetValue.new(tracker, 0.45, 1.1, &"smooth"),
		GShapes.SetValue.new(tracker, 1.0, 1.5, &"smooth"),
		GShapes.SetValue.new(tracker, 0.3, 1.0, &"linear"),
	])
