...
func _build_report() -> void:
	...
	for scene_path in all_scenes:
		var phase: String = GShapes.PhaseCoverage.phase_tag_for_scene(scene_path)
		phase_counts[phase] = int(phase_counts.get(phase, 0)) + 1
		if not ResourceLoader.exists(scene_path):
			missing_resources.append(scene_path)

	var ratio: Vector2i = GShapes.PhaseCoverage.coverage_ratio(phase_counts, true)
	summary_label.text = "Coverage: phase6_full=%d/%d | visual=%d | tools=%d | missing=%d" % [
		ratio.x,
		ratio.y,
		GShapes.DemoCatalog.get_visual_demos().size(),
		GShapes.DemoCatalog.TOOL_DEMOS.size(),
		missing_resources.size(),
	]

	var lines: Array[String] = []
	var wanted: Array[String] = GShapes.PhaseCoverage.wanted_phase_tags(true)
	for tag in wanted:
