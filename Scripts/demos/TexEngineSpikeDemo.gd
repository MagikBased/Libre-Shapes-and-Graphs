...
func _ready() -> void:
	...
	equation = GShapes.MathTexMobject.new()
	equation.tex_engine_mode = GShapes.TexMobject.TexEngineMode.EXTERNAL_SPIKE_AUTO
	...
	play(GShapes.Write.new(equation, 1.2, &"linear"))
...
func _update_status_from_report(report: Dictionary) -> void:
	...
	var cache_stats: Dictionary = GShapes.TexToolchainSpike.get_cache_stats()
