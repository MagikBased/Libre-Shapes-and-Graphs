...
func _ready() -> void:
	...
	play_sequence([
		GShapes.SetValue.new(phase_tracker, PI * 1.1, 2.0, &"smooth"),
		GShapes.SetValue.new(phase_tracker, TAU * 1.75, 2.0, &"linear"),
		GShapes.SetValue.new(phase_tracker, -PI * 0.5, 2.0, &"smooth"),
	])


func _create_numeric_panel() -> void:
	_make_key_label("theta:", Vector2(44.0, 84.0))
	theta_value = GShapes.DecimalNumber.new(0.0, 3, true, " rad")
	...
	sin_value = GShapes.DecimalNumber.new(0.0, 4, true, "")
	...
	cos_value = GShapes.DecimalNumber.new(0.0, 4, true, "")
