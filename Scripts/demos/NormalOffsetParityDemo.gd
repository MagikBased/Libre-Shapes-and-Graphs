...
var offset_curve: GShapesNormalOffsetCurve2D
...
func _ready() -> void:
	...
	offset_curve = GShapes.NormalOffsetCurve2D.new()
	...
	play(GShapes.FadeIn.new(offset_curve, 0.3, &"smooth"))
	wait(0.2)
	play_sequence([
		GShapes.SetValue.new(tracker, 0.48, 1.2, &"smooth"),
		GShapes.SetValue.new(tracker, 1.0, 1.4, &"smooth"),
		GShapes.SetValue.new(tracker, 0.32, 1.1, &"linear"),
	])
