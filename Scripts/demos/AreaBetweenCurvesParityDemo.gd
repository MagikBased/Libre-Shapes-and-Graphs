...
var band: GShapesAreaBetweenCurves2D
...
func _ready() -> void:
	...
	band = GShapes.AreaBetweenCurves2D.new()
	band.axes = axes
	...
	tracker = GShapes.ValueTracker.new(-1.4)
	add_child(tracker)
	...
	play_group([
		GShapes.ShowCreation.new(sin_plot, 1.1, &"smooth"),
		GShapes.ShowCreation.new(cos_plot, 1.1, &"smooth"),
		GShapes.FadeIn.new(band, 0.5, &"smooth"),
	])
	play_sequence([
		GShapes.SetValue.new(tracker, 2.7, 1.35, &"smooth"),
		GShapes.SetValue.new(tracker, 5.2, 1.1, &"smooth"),
		GShapes.SetValue.new(tracker, -0.8, 1.55, &"there_and_back_with_pause"),
	])
