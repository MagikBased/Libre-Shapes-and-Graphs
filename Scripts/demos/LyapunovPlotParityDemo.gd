...
func _ready() -> void:
	...
	zero_line = GShapes.PolylineMobject.new()
	...
	lyapunov_plot = GShapes.LyapunovPlot2D.new()
	...
	play(GShapes.ShowCreation.new(lyapunov_plot, 1.1, &"smooth"))
	play_sequence([
		GShapes.SetValue.new(seed_tracker, 0.62, 1.15, &"smooth"),
		GShapes.SetValue.new(seed_tracker, 0.21, 1.25, &"there_and_back_with_pause"),
		GShapes.SetValue.new(seed_tracker, 0.44, 1.0, &"smooth"),
	])
