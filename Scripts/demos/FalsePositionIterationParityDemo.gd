...
var false_pos: GShapesFalsePositionIteration2D
var k_tracker: GShapesValueTracker
var left_tracker: GShapesValueTracker
var right_tracker: GShapesValueTracker
...
func _ready() -> void:
	...
	false_pos = GShapes.FalsePositionIteration2D.new()
	false_pos.axes = axes
	...
	play(GShapes.FadeIn.new(false_pos, 0.35, &"smooth"))
	play_sequence([
		GShapes.SetValue.new(left_tracker, -2.5, 1.2, &"smooth"),
		GShapes.SetValue.new(right_tracker, 1.9, 1.2, &"smooth"),
		GShapes.SetValue.new(k_tracker, -0.22, 1.35, &"there_and_back_with_pause"),
		GShapes.SetValue.new(left_tracker, -3.1, 1.0, &"smooth"),
	])
