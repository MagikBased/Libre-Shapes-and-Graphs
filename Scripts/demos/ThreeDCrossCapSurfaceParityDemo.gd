# Demo: ThreeDCrossCapSurfaceParityDemo
# Expected behavior: See PlansAndCopy/DEMO_NOTES.md

extends GShapes3DScene
var axes: GShapesAxes3D
var cross_cap: GShapesCrossCapSurface3D
...
func _ready() -> void:
	...
	cross_cap = GShapes.CrossCapSurface3D.new()
	cross_cap.mode_name = modes[mode_index]
	cross_cap.u_steps = 104
	cross_cap.v_steps = 78
	cross_cap.scale_factor = 1.22
	cross_cap.surface_color = colors[mode_index]
	add_child(cross_cap)
...
func _process(delta: float) -> void:
	...
	cross_cap.phase = time_accum
	cross_cap.rotation.y += delta * 0.22
	cross_cap.rotation.x = sin(time_accum * 0.34) * 0.08
...
func _frame_all() -> void:
	var nodes: Array = [axes, cross_cap]
...
func _create_overlay() -> void:
	...
	add_child(layer)
	var label := Label.new()
	label.text = "3D cross-cap parity: GShapesCrossCapSurface3D | Space/1/2/3 mode, RMB orbit, MMB pan, wheel zoom, R reset, F reframe"
