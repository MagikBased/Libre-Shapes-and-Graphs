...
var network: GShapesCurveNetwork3D
...
func _create_overlay() -> void:
	...
	add_child(layer)
	var label := Label.new()
	label.text = "3D curve-network parity: GShapesCurveNetwork3D | Space/1/2/3 mode, RMB orbit, MMB pan, wheel zoom, R reset, F reframe"
