...
var recurrence_plot: GShapesRecurrencePlot2D
...
func _ready() -> void:
	...
	recurrence_plot = GShapes.RecurrencePlot2D.new()
	...
	play(GShapes.FadeIn.new(recurrence_plot, 0.5, &"smooth"))
	play_sequence([
		GShapes.SetValue.new(parameter_tracker, 3.70, 1.2, &"smooth"),
		GShapes.SetValue.new(parameter_tracker, 3.90, 1.3, &"there_and_back_with_pause"),
