...
func _ready() -> void:
	...
	highlight = GShapes.SurroundingRectangle2D.new(labels[0], Vector2(20.0, 10.0))
...
func _focus_index(index: int) -> void:
	...
	play(GShapes.FadeToColor.new(labels[active_index], labels[active_index].color, Color(0.5, 0.9, 1.0), 0.25, &"smooth"))
	wait(0.05)
	play(GShapes.FadeToColor.new(labels[active_index], labels[active_index].color, Color(0.92, 0.96, 1.0), 0.25, &"smooth"))
