...
func _ready() -> void:
	...
	play(GShapes.ShowCreation.new(parabola, 1.4, &"smooth"))
	wait_seconds(0.2)
	play(GShapes.FadeIn.new(dot, 0.3, &"smooth"))
	play_sequence([
		GShapes.MoveTo.new(x_controller, Vector2(4.0, 0.0), 2.0, &"smooth"),
		GShapes.MoveTo.new(x_controller, Vector2(-2.0, 0.0), 2.0, &"smooth"),
	])
