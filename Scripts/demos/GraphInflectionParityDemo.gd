...
var inflections: GShapesGraphInflectionPoints2D
...
func _ready() -> void:
	...
	phase_tracker = GShapes.ValueTracker.new(0.2)
	...
	inflections = GShapes.GraphInflectionPoints2D.new()
	...
	play(GShapes.FadeIn.new(inflections, 0.4, &"smooth"))
	play_sequence([
		GShapes.SetValue.new(phase_tracker, 1.4, 1.4, &"smooth"),
		GShapes.SetValue.new(phase_tracker, -0.6, 1.35, &"there_and_back_with_pause"),
		GShapes.SetValue.new(phase_tracker, 2.0, 1.5, &"smooth"),
	])
