...
func _ready() -> void:
	...
	tracker = GShapes.ValueTracker.new(-4.2)
	add_child(tracker)

	sin_follower = GShapes.GraphLabelFollower2D.new()
	...
	cos_follower = GShapes.GraphLabelFollower2D.new()
	...
	play_group([
		GShapes.ShowCreation.new(sin_plot, 1.2, &"smooth"),
		GShapes.ShowCreation.new(cos_plot, 1.2, &"smooth"),
		GShapes.FadeIn.new(sin_follower, 0.5, &"smooth"),
		GShapes.FadeIn.new(cos_follower, 0.5, &"smooth"),
	])
	play_sequence([
		GShapes.SetValue.new(tracker, 4.8, 1.6, &"smooth"),
		GShapes.SetValue.new(tracker, -5.0, 1.7, &"smooth"),
		GShapes.SetValue.new(tracker, 1.8, 1.1, &"there_and_back_with_pause"),
	])
