...
func _ready() -> void:
	...
	roman = GShapes.RomanSurface3D.new()
	roman.mode_name = modes[mode_index]
	roman.u_steps = 96
	roman.v_steps = 72
	roman.scale_factor = 2.5
	roman.surface_color = colors[mode_index]
	add_child(roman)
...
func _process(delta: float) -> void:
	...
	roman.phase = time_accum
	roman.rotation.y += delta * 0.2
	roman.rotation.x = sin(time_accum * 0.37) * 0.08
...
func _create_overlay() -> void:
	...
	add_child(layer)
	var label := Label.new()
	label.text = "3D roman-surface parity: GShapesRomanSurface3D | Space/1/2/3 mode, RMB orbit, MMB pan, wheel zoom, R reset, F reframe"
