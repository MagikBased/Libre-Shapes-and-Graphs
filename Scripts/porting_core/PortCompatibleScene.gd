...
func move_camera(
	...
	play(PortMoveTo.new(camera, to_world_position, run_time, rate_func_name))
...
func zoom_camera(
	...
	var camera := _ensure_scene_camera()
	play(PortCameraZoomTo.new(camera, to_zoom, run_time, rate_func_name))
...
func zoom_to_fit(
	...
	play([
		PortMoveTo.new(_ensure_scene_camera(), center, run_time, rate_func_name),
		PortCameraZoomTo.new(_ensure_scene_camera(), Vector2.ONE * zoom_factor, run_time, rate_func_name),
	])
