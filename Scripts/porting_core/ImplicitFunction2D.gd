...
func recompute_segments() -> void:
	...
	var n := maxi(8, grid_resolution)
	for ix in range(n - 1):
		for iy in range(n - 1):
			var x0 := lerpf(axes.x_min, axes.x_max, float(ix) / float(n - 1))
			var x1 := lerpf(axes.x_min, axes.x_max, float(ix + 1) / float(n - 1))
			var y0 := lerpf(axes.y_min, axes.y_max, float(iy) / float(n - 1))
			var y1 := lerpf(axes.y_min, axes.y_max, float(iy + 1) / float(n - 1))

			var f00 := _eval_f(x0, y0)
			var f10 := _eval_f(x1, y0)
			var f01 := _eval_f(x0, y1)
			var f11 := _eval_f(x1, y1)
			...
			if crossings.size() >= 2:
				for i in range(0, crossings.size() - 1, 2):
					var a := axes.graph_to_local(crossings[i])
					var b := axes.graph_to_local(crossings[i + 1])
...
func _draw() -> void:
	if _segments.is_empty():
		return
	var max_idx := int(floor(float(_segments.size()) * draw_progress))
...
func _add_edge_crossing(crossings: Array[Vector2], a: Vector2, b: Vector2, fa: float, fb: float) -> void:
	if (fa > 0.0 and fb > 0.0) or (fa < 0.0 and fb < 0.0):
		return
	var denom := fb - fa
	var t := 0.5 if is_zero_approx(denom) else clampf(-fa / denom, 0.0, 1.0)
