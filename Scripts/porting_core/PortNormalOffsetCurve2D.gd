class_name PortNormalOffsetCurve2D
extends PortPolylineMobject

var source_curve: PortPolylineMobject
...
func rebuild_offset() -> void:
	...
	for i in range(count):
		...
		var p: Vector2 = PortPathUtils.sample_polyline(src, a, false)
		var eps: float = 0.003
		var a_prev: float = clampf(a - eps, 0.0, 1.0)
		var a_next: float = clampf(a + eps, 0.0, 1.0)
		var p_prev: Vector2 = PortPathUtils.sample_polyline(src, a_prev, false)
		var p_next: Vector2 = PortPathUtils.sample_polyline(src, a_next, false)
