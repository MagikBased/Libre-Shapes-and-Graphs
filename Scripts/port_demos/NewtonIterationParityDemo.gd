...
var newton: PortNewtonIteration2D
...
func _ready() -> void:
	...
	k_tracker = PortValueTracker.new(0.45)
	x0_tracker = PortValueTracker.new(2.6)
	add_child(k_tracker)
	...
	newton = PortNewtonIteration2D.new()
	newton.axes = axes
	...
	play(PortFadeIn.new(newton, 0.35, &"smooth"))
	play_sequence([
		PortSetValue.new(x0_tracker, -2.2, 1.35, &"smooth"),
		PortSetValue.new(k_tracker, -0.3, 1.35, &"there_and_back_with_pause"),
		PortSetValue.new(x0_tracker, 1.4, 1.2, &"smooth"),
	])
