...
func _ready() -> void:
	...
	play_group([
		PortGrowFromCenter.new(c_grow, 0.9, &"overshoot"),
		PortWiggle.new(c_wiggle, 1.2, &"smooth", 3.0),
		PortRotate.new(c_rotate, TAU * 0.75, 1.1, &"smooth", true),
		PortShrinkToCenter.new(c_shrink, 0.9, &"smooth"),
	])
	wait(0.2)
	play([
