...
var connectors: Array[PortDashedLine3D] = []
...
func _update_connectors() -> void:
	if movers.size() < 3 or connectors.size() < 3:
		return
	var c0: PortDashedLine3D = connectors[0]
	var c1: PortDashedLine3D = connectors[1]
	var c2: PortDashedLine3D = connectors[2]
...
func _create_overlay() -> void:
	...
	add_child(layer)
	var label := Label.new()
	label.text = "3D dashed-line parity: PortDashedLine3D connectors | RMB orbit, MMB pan, wheel zoom, R reset, F reframe"
