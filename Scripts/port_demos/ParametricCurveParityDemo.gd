...
var param_curve: PortParametricFunction2D
...
func _ready() -> void:
	...
	param_curve = PortParametricFunction2D.new()
	...
	play(PortShowCreation.new(param_curve, 1.4, &"smooth"))
	play(PortFadeIn.new(dot, 0.45, &"smooth"))
	wait(0.2)
	play_sequence([
		PortSetValue.new(tracker, TAU * 0.45, 1.3, &"smooth"),
		PortSetValue.new(tracker, TAU * 0.92, 1.3, &"smooth"),
		PortSetValue.new(tracker, TAU * 1.25, 1.4, &"linear"),
	])
