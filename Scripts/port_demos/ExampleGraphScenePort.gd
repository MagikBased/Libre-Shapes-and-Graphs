...
func _ready() -> void:
	_create_caption("Port of GraphExample: graph creation, replacement, and point motion")
	...
	play(PortShowCreation.new(active_plot, 1.2, &"smooth"))
	wait_seconds(1.0)

	var parabola := _make_plot(&"parabola", Color.ORANGE_RED)
	play(PortFadeOut.new(active_plot, 0.6, &"smooth"))
	play(PortFadeIn.new(parabola, 0.6, &"smooth"))
	wait_seconds(0.5)
	...
	play(PortFadeIn.new(dot, 0.4, &"smooth"))
	wait_seconds(0.2)

	play(PortMoveTo.new(dot, _parabola_world_point(4.0), 1.8, &"smooth"))
	wait_seconds(0.1)
	play(PortMoveTo.new(dot, _parabola_world_point(-2.0), 1.8, &"smooth"))
