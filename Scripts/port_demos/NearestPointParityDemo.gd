...
var probe: Circle
var nearest: PortNearestPointOnCurve2D
...
func _ready() -> void:
	...
	add_child(probe)
	probe.add_updater(func(target: PortObject2D, _delta: float) -> void:
	...
	nearest = PortNearestPointOnCurve2D.new()
	...
	play(PortFadeIn.new(probe, 0.25, &"smooth"))
	play(PortFadeIn.new(nearest, 0.25, &"smooth"))
	wait(0.2)
	play_sequence([
		PortSetValue.new(tracker, 0.45, 1.2, &"smooth"),
		PortSetValue.new(tracker, 1.0, 1.45, &"smooth"),
		PortSetValue.new(tracker, 0.22, 1.15, &"linear"),
	])
