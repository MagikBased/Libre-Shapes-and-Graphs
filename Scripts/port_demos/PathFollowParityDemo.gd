...
func _ready() -> void:
	...
	var path := PortPolylineMobject.new()
	...
	var mover := PortPolylineMobject.new()
	...
	var tracer := PortTracedPath2D.new()
	...
	var sweep := PortMoveAlongPath2D.new(
		mover,
		path.points,
		8.0,
		&"linear",
	...
	wait(0.25)
	play(PortFadeToColor.new(path, path.color, Color(0.95, 0.76, 0.28, 0.95), 0.7, &"smooth"))
