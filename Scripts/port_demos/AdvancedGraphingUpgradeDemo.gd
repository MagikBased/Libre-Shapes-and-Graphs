...
var area: PortAreaUnderCurve2D
var guides: PortGraphGuideLines2D
var tangent_line: PortPolylineMobject
var secant_line: PortPolylineMobject
var dot: Circle
...
func _ready() -> void:
	...
	guides = PortGraphGuideLines2D.new()
	guides.axes = axes
	...
	tangent_line = PortPolylineMobject.new()
	...
	secant_line = PortPolylineMobject.new()
	...
	play(PortShowCreation.new(plot, 1.2, &"smooth"))
	play_group([
		PortShowCreation.new(area, 1.0, &"smooth"),
		PortShowCreation.new(guides, 1.0, &"smooth"),
		PortShowCreation.new(tangent_line, 1.0, &"smooth"),
		PortShowCreation.new(secant_line, 1.0, &"smooth"),
		PortFadeIn.new(dot, 0.3, &"smooth"),
	])
	play_sequence([
		PortSetValue.new(x_tracker, 4.5, 1.6, &"smooth"),
		PortSetValue.new(x_tracker, 0.2, 1.4, &"there_and_back_with_pause"),
	])


func _update_marker_system(target: PortObject2D, _delta: float) -> void:
