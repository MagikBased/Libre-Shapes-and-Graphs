...
func _build_report() -> void:
	var all_scenes: Array[String] = PortDemoCatalog.get_visual_demos()
	...
	for scene_path in all_scenes:
		var phase8: String = PortPhaseCoverage.phase8_tag_for_scene(scene_path)
		if phase8 != "base":
			phase8_counts[phase8] = int(phase8_counts.get(phase8, 0)) + 1
			if not ResourceLoader.exists(scene_path):
				missing_resources.append(scene_path)

	var visual_ratio: Vector2i = PortPhaseCoverage.coverage_ratio_phase8(phase8_counts, false)
	var full_ratio: Vector2i = PortPhaseCoverage.coverage_ratio_phase8(phase8_counts, true)
	...
	var wanted: Array[String] = PortPhaseCoverage.wanted_phase8_tags(true)
	for tag in wanted:
