...
var plane: PortPolarPlane2D
var curve: PortPolarPlot2D
var marker: Circle
...
func _ready() -> void:
	...
	play(PortFadeIn.new(marker, 0.35, &"smooth"))
	wait(0.15)
	play_sequence([
		PortSetValue.new(tracker, TAU * 0.45, 1.0, &"smooth"),
		PortSetValue.new(tracker, TAU * 0.95, 1.15, &"smooth"),
		PortSetValue.new(tracker, TAU * 1.3, 1.2, &"linear"),
	])
