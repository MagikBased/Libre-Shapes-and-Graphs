...
# Expected behavior: PortCompatibleScene-style timeline API running inside flat 2D-on-3D scene mode.

extends PortFlatCompatibleScene3D
...
func _run_timeline() -> void:
	play(PortShowCreation.new(connector, 0.7, &"linear"))
	play_lagged([
		PortGrowFromCenter.new(circle, 0.6, &"smooth"),
		PortGrowFromCenter.new(square, 0.6, &"smooth"),
	], 0.2, 1.1, &"smooth")
	wait(0.2)
	play([
	...
	play_map([circle, square], func(node: Node2D):
		return (node as PortObject2D).animate.rotate_to(node.rotation + 0.5).set_run_time(0.7)
	)
	wait(0.1)
	play(PortWiggle.new(circle, 0.8, &"smooth"))
	play(PortWiggle.new(square, 0.8, &"smooth"))
