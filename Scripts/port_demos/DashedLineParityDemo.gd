...
func _ready() -> void:
	...
	dashed = PortDashedLine2D.new()
	dashed.color = Color(0.86, 0.92, 1.0)
	dashed.stroke_width = 4.0
	dashed.dash_length = 22.0
	dashed.gap_length = 12.0
	add_child(dashed)
	dashed.add_updater(func(target: PortObject2D, _delta: float) -> void:
		(target as PortDashedLine2D).set_endpoints(start_dot.position, end_dot.position)
	)

	end_dot.add_updater(func(target: PortObject2D, _delta: float) -> void:
	...
	play(PortShowCreation.new(dashed, 1.2, &"smooth"))
	wait(0.2)
	play_sequence([
		PortSetValue.new(phase_tracker, TAU * 1.2, 1.6, &"smooth"),
		PortSetValue.new(phase_tracker, TAU * 2.3, 1.7, &"linear"),
		PortSetValue.new(phase_tracker, TAU * 3.1, 1.5, &"smooth"),
	])
