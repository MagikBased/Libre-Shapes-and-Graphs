...
var axes: PortAxes3D
var network: PortCurveNetwork3D
...
func _ready() -> void:
	...
	network = PortCurveNetwork3D.new()
...
func _create_overlay() -> void:
	...
	add_child(layer)
	var label := Label.new()
	label.text = "3D curve-network parity: PortCurveNetwork3D | Space/1/2/3 mode, RMB orbit, MMB pan, wheel zoom, R reset, F reframe"
