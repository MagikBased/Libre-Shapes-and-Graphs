...
var axes: PortAxes3D
var surface: PortSurfaceMesh3D
var cloud: PortPointCloud3D
...
func _run_effect_timeline() -> void:
	...
	chain.tween_callback(func(): PortEffects3D.pulse_scale(self, cloud, 1.26, 0.9, 2))
	chain.tween_interval(1.0)
	chain.tween_callback(func(): PortEffects3D.spin_y(self, surface, PI * 1.2, 1.0))
	chain.tween_interval(0.35)
	chain.tween_callback(func(): PortEffects3D.staggered_pulse(self, effect_nodes, 0.07, 1.22, 0.46))
	chain.tween_interval(0.9)
	chain.tween_callback(func():
		if effect_nodes.size() > 0:
			PortEffects3D.arc_move_to(self, effect_nodes[0], Vector3(0.0, 0.85, -2.6), 0.75, 1.1)
	)
	chain.tween_interval(0.5)
	chain.tween_callback(func():
		if effect_nodes.size() > 0:
			PortEffects3D.arc_move_to(self, effect_nodes[0], Vector3(2.1, 0.35, 0.0), 0.75, 1.1)
	)
