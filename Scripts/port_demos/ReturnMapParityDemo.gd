...
var return_map: PortReturnMap2D
var parameter_tracker: PortValueTracker
var diagonal: PortPolylineMobject
...
func _ready() -> void:
	...
	diagonal = PortPolylineMobject.new()
	...
	parameter_tracker = PortValueTracker.new(3.6)
	add_child(parameter_tracker)

	return_map = PortReturnMap2D.new()
	...
	play(PortFadeIn.new(return_map, 0.5, &"smooth"))
	play_sequence([
		PortSetValue.new(parameter_tracker, 3.74, 1.2, &"smooth"),
		PortSetValue.new(parameter_tracker, 3.89, 1.3, &"there_and_back_with_pause"),
		PortSetValue.new(parameter_tracker, 3.64, 1.0, &"smooth"),
	])
