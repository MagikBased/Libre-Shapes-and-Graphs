...
var traced_path: PortTracedPath2D
...
func _ready() -> void:
	...
	traced_path = PortTracedPath2D.new()
	...
	var dynamic_line := PortAlwaysRedraw2D.new(func():
		var line := Line.new()
	...
	moving_dot.add_updater(func(node: PortObject2D, _delta: float) -> void:
	...
	play_sequence([
		PortSetValue.new(tracker, TAU * 2.0, 3.0, &"linear"),
		PortSetValue.new(tracker, TAU * 4.5, 3.0, &"linear"),
		PortSetValue.new(tracker, TAU * 6.8, 3.2, &"linear"),
	])


func _make_orbit_frame(center: Vector2) -> PortPolylineMobject:
	var frame := PortPolylineMobject.new()
