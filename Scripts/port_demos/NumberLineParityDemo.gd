...
var number_line: PortNumberLine2D
var tracker: PortValueTracker
var pointer: Circle
var value_readout: PortDecimalNumber


func _ready() -> void:
	_create_caption("Phase 6 number-line parity: ticks/labels + tracker-driven pointer")

	tracker = PortValueTracker.new(-4.0)
	add_child(tracker)

	number_line = PortNumberLine2D.new()
	...
	add_child(pointer)
	pointer.add_updater(func(target: PortObject2D, _delta: float) -> void:
	...
	value_readout = PortDecimalNumber.new(tracker.get_value(), 2, true, "")
	...
	play_sequence([
		PortSetValue.new(tracker, 2.5, 1.5, &"smooth"),
		PortSetValue.new(tracker, -1.0, 1.2, &"smooth"),
		PortSetValue.new(tracker, 7.25, 1.7, &"linear"),
	])
