...
var extrema: PortGraphExtrema2D
...
func _ready() -> void:
	...
	extrema = PortGraphExtrema2D.new()
	extrema.axes = axes
	...
	play(PortFadeIn.new(extrema, 0.4, &"smooth"))
	play_sequence([
		PortSetValue.new(phase_tracker, 1.2, 1.4, &"smooth"),
		PortSetValue.new(phase_tracker, -0.7, 1.35, &"there_and_back_with_pause"),
		PortSetValue.new(phase_tracker, 2.1, 1.6, &"smooth"),
	])
