...
var seq_plot: PortIterationSequencePlot2D
...
func _ready() -> void:
	...
	x0_tracker = PortValueTracker.new(1.4)
	add_child(k_tracker)
	add_child(x0_tracker)

	seq_plot = PortIterationSequencePlot2D.new()
	seq_plot.axes = axes
	...
	play(PortShowCreation.new(seq_plot, 1.1, &"smooth"))
	play(PortFadeIn.new(end_marker, 0.35, &"smooth"))
	play_sequence([
		PortSetValue.new(x0_tracker, -0.8, 1.25, &"smooth"),
		PortSetValue.new(k_tracker, -0.22, 1.35, &"there_and_back_with_pause"),
		PortSetValue.new(x0_tracker, 0.6, 1.1, &"smooth"),
	])
