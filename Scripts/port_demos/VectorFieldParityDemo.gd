...
var field: PortVectorField2D
var strength_tracker: PortValueTracker
var strength_label: PortDecimalNumber


func _ready() -> void:
	_create_caption("Phase 6 vector-field parity: sampled arrow field with animated strength")

	strength_tracker = PortValueTracker.new(0.8)
	add_child(strength_tracker)

	field = PortVectorField2D.new()
	...
	strength_label = PortDecimalNumber.new(strength_tracker.get_value(), 2, false, "")
	...
	play_sequence([
		PortSetValue.new(strength_tracker, 1.6, 1.5, &"smooth"),
		PortSetValue.new(strength_tracker, 0.45, 1.4, &"smooth"),
		PortSetValue.new(strength_tracker, 1.2, 1.4, &"linear"),
	])
