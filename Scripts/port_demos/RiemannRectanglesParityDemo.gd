...
var rectangles: PortRiemannRectangles2D
var x_tracker: PortValueTracker
var dx_tracker: PortValueTracker
var mode_tracker: PortValueTracker
...
func _ready() -> void:
	...
	rectangles = PortRiemannRectangles2D.new()
	...
	x_tracker = PortValueTracker.new(3.2)
	dx_tracker = PortValueTracker.new(1.0)
	mode_tracker = PortValueTracker.new(0.0)
	add_child(x_tracker)
	...
	play(PortShowCreation.new(plot, 1.0, &"smooth"))
	play(PortFadeIn.new(rectangles, 0.55, &"smooth"))
	play_sequence([
		PortSetValue.new(x_tracker, 5.8, 1.3, &"smooth"),
		PortSetValue.new(dx_tracker, 0.25, 1.45, &"smooth"),
		PortSetValue.new(mode_tracker, 1.0, 0.8, &"smooth"),
		PortSetValue.new(mode_tracker, 2.0, 0.8, &"smooth"),
		PortSetValue.new(x_tracker, 2.2, 1.0, &"there_and_back_with_pause"),
	])


func _update_rectangles(_target: PortObject2D, _delta: float) -> void:
