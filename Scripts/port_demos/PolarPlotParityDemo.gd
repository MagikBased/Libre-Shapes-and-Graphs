...
var polar_curve: PortPolarPlot2D
var marker: Circle
...
func _ready() -> void:
	...
	polar_curve = PortPolarPlot2D.new()
	...
	play(PortShowCreation.new(polar_curve, 1.2, &"smooth"))
	play(PortFadeIn.new(marker, 0.4, &"smooth"))
	wait(0.2)
	play_sequence([
		PortSetValue.new(tracker, TAU * 0.4, 1.1, &"smooth"),
		PortSetValue.new(tracker, TAU * 0.85, 1.1, &"smooth"),
		PortSetValue.new(tracker, TAU * 1.2, 1.2, &"linear"),
	])
