...
var trail_a: PortTracedPath3D
var trail_b: PortTracedPath3D
...
func _create_trail(color: Color, width_value: float, max_points_value: int) -> PortTracedPath3D:
	var trail := PortTracedPath3D.new()
...
func _create_overlay() -> void:
	...
	add_child(layer)
	var label := Label.new()
	label.text = "3D traced-path parity: animated PortTracedPath3D trails | RMB orbit, MMB pan, wheel zoom, R reset, F reframe, C clear"
