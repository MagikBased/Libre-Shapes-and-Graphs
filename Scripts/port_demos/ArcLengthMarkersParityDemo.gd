...
var markers: PortArcLengthMarkers2D
...
func _ready() -> void:
	...
	markers = PortArcLengthMarkers2D.new()
	...
	play(PortFadeIn.new(markers, 0.3, &"smooth"))
	wait(0.2)
	play_sequence([
		PortSetValue.new(tracker, 0.55, 1.15, &"smooth"),
		PortSetValue.new(tracker, 1.0, 1.35, &"smooth"),
		PortSetValue.new(tracker, 0.3, 1.1, &"linear"),
	])
