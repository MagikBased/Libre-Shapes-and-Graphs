...
var bisection: PortBisectionIteration2D
...
func _ready() -> void:
	...
	bisection = PortBisectionIteration2D.new()
	bisection.axes = axes
	...
	play(PortFadeIn.new(bisection, 0.35, &"smooth"))
	play_sequence([
		PortSetValue.new(left_tracker, -2.4, 1.2, &"smooth"),
		PortSetValue.new(right_tracker, 1.8, 1.2, &"smooth"),
		PortSetValue.new(k_tracker, -0.25, 1.35, &"there_and_back_with_pause"),
		PortSetValue.new(left_tracker, -3.0, 1.0, &"smooth"),
	])
