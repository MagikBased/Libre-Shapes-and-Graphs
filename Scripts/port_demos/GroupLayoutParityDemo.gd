...
func _ready() -> void:
	...
	play_lagged_map(
		...
		func(member: Node2D, i: int):
			return PortMoveTo.new(member, line_targets[i], 0.9, &"smooth"),
		0.07,
		1.1,
		&"smooth"
	)
	...
	play_map(
		group.get_members(),
		func(member: Node2D, i: int):
			return PortMoveTo.new(member, grid_targets[i], 1.0, &"smooth"),
		1.0,
		&"smooth"
	)
	...
	play_lagged_map(
		group.get_members(),
		func(member: Node2D, i: int):
			return (member as PortObject2D).animate.set_color(Color.from_hsv(float(i) / 12.0, 0.7, 1.0)),
		0.05,
		0.8,
		&"smooth"
	)
