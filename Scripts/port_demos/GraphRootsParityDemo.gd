...
var dynamic_curve: PortParametricFunction2D
var roots_helper: PortGraphRoots2D
...
func _ready() -> void:
	...
	k_tracker = PortValueTracker.new(0.7)
	add_child(k_tracker)

	dynamic_curve = PortParametricFunction2D.new()
	...
	roots_helper = PortGraphRoots2D.new()
	...
	play(PortShowCreation.new(dynamic_curve, 1.15, &"smooth"))
	play(PortFadeIn.new(roots_helper, 0.35, &"smooth"))
	play_sequence([
		PortSetValue.new(k_tracker, -0.35, 1.45, &"smooth"),
		PortSetValue.new(k_tracker, 0.15, 1.1, &"there_and_back_with_pause"),
		PortSetValue.new(k_tracker, -0.75, 1.35, &"smooth"),
	])
