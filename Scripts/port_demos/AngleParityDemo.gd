...
var angle_arc: PortAngle2D
var angle_label: PortDecimalNumber
var theta_tracker: PortValueTracker
...
func _ready() -> void:
	...
	theta_tracker = PortValueTracker.new(deg_to_rad(25.0))
	...
	add_child(ray_a)
	ray_a.add_updater(func(target: PortObject2D, _delta: float) -> void:
	...
	angle_label.add_updater(func(target: PortObject2D, _delta: float) -> void:
		(target as Node2D).global_position = angle_arc.get_label_anchor(26.0) + Vector2(-44.0, -16.0)
	)

	play_sequence([
		PortSetValue.new(theta_tracker, deg_to_rad(95.0), 1.3, &"smooth"),
		PortSetValue.new(theta_tracker, deg_to_rad(145.0), 1.3, &"smooth"),
		PortSetValue.new(theta_tracker, deg_to_rad(40.0), 1.3, &"smooth"),
	])
