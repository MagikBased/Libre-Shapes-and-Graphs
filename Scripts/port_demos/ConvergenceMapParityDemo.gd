...
var conv_map: PortConvergenceMap2D
...
func _ready() -> void:
	...
	parameter_tracker = PortValueTracker.new(3.2)
	add_child(parameter_tracker)

	conv_map = PortConvergenceMap2D.new()
	conv_map.axes = axes
	...
	play(PortFadeIn.new(conv_map, 0.45, &"smooth"))
	play_sequence([
		PortSetValue.new(parameter_tracker, 3.45, 1.2, &"smooth"),
		PortSetValue.new(parameter_tracker, 3.72, 1.2, &"smooth"),
		PortSetValue.new(parameter_tracker, 3.88, 1.25, &"there_and_back_with_pause"),
		PortSetValue.new(parameter_tracker, 3.56, 1.0, &"smooth"),
	])
