...
var secant_iter: PortSecantIteration2D
var k_tracker: PortValueTracker
var x0_tracker: PortValueTracker
var x1_tracker: PortValueTracker
...
func _ready() -> void:
	...
	x0_tracker = PortValueTracker.new(-2.0)
	x1_tracker = PortValueTracker.new(2.8)
	add_child(k_tracker)
	...
	secant_iter = PortSecantIteration2D.new()
	...
	play(PortFadeIn.new(secant_iter, 0.35, &"smooth"))
	play_sequence([
		PortSetValue.new(x0_tracker, -3.0, 1.2, &"smooth"),
		PortSetValue.new(x1_tracker, 2.0, 1.2, &"smooth"),
		PortSetValue.new(k_tracker, -0.25, 1.35, &"there_and_back_with_pause"),
		PortSetValue.new(x1_tracker, 1.2, 1.0, &"smooth"),
	])
