...
var density_plot: PortInvariantDensity2D
...
func _ready() -> void:
	...
	parameter_tracker = PortValueTracker.new(3.72)
	add_child(parameter_tracker)

	density_plot = PortInvariantDensity2D.new()
	...
	play(PortFadeIn.new(density_plot, 0.45, &"smooth"))
	play_sequence([
		PortSetValue.new(parameter_tracker, 3.87, 1.3, &"smooth"),
		PortSetValue.new(parameter_tracker, 3.61, 1.2, &"there_and_back_with_pause"),
		PortSetValue.new(parameter_tracker, 3.76, 1.0, &"smooth"),
	])
