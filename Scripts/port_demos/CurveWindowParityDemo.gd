...
var window_curve: PortCurveWindow2D
var tip: Circle
...
func _ready() -> void:
	...
	window_curve = PortCurveWindow2D.new()
	...
	play(PortFadeIn.new(window_curve, 0.3, &"smooth"))
	play(PortFadeIn.new(tip, 0.25, &"smooth"))
	wait(0.2)
	play_sequence([
		PortSetValue.new(tracker, 0.42, 1.2, &"smooth"),
		PortSetValue.new(tracker, 1.0, 1.45, &"smooth"),
		PortSetValue.new(tracker, 0.28, 1.15, &"linear"),
	])
