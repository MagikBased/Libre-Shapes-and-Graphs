...
var secant_group: PortSecantSlopeGroup2D
...
func _ready() -> void:
	...
	x_tracker = PortValueTracker.new(-2.6)
	dx_tracker = PortValueTracker.new(1.5)
	add_child(x_tracker)
	add_child(dx_tracker)

	secant_group = PortSecantSlopeGroup2D.new()
	...
	play(PortShowCreation.new(plot, 1.1, &"smooth"))
	play(PortFadeIn.new(secant_group, 0.35, &"smooth"))
	play_sequence([
		PortSetValue.new(x_tracker, 5.2, 1.7, &"smooth"),
		PortSetValue.new(dx_tracker, 0.25, 1.25, &"smooth"),
		PortSetValue.new(x_tracker, -0.5, 1.2, &"there_and_back_with_pause"),
		PortSetValue.new(dx_tracker, 1.3, 1.0, &"smooth"),
	])
