...
var chain: PortEpicycleChain2D
var tracer: PortTracedPath2D
var dot: Circle
...
func _ready() -> void:
	...
	play(PortFadeIn.new(chain, 0.4, &"smooth"))
	play(PortFadeIn.new(dot, 0.25, &"smooth"))
	wait(0.15)
	play_sequence([
		PortSetValue.new(tracker, TAU * 0.9, 1.45, &"smooth"),
		PortSetValue.new(tracker, TAU * 1.75, 1.35, &"smooth"),
		PortSetValue.new(tracker, TAU * 2.4, 1.35, &"linear"),
	])
