...
func _ready() -> void:
	...
	x_tracker = PortValueTracker.new(2.0)
	add_child(x_tracker)
	...
	play(PortShowCreation.new(parabola, 1.2, &"smooth"))
	wait_seconds(0.2)
	play(PortFadeIn.new(dot, 0.3, &"smooth"))
	play_sequence([
		PortSetValue.new(x_tracker, 4.0, 1.8, &"smooth"),
		PortSetValue.new(x_tracker, -2.0, 1.8, &"smooth"),
	])
