...
func _ready() -> void:
	...
	x_tracker = PortValueTracker.new(-5.0)
	add_child(x_tracker)
	...
	play(PortFadeIn.new(dot, 0.35, &"smooth"))
	wait(0.2)
	zoom_to_fit([axes_rect], 0.08, 1.0, &"smooth")
	wait(0.2)
	play(PortSetValue.new(x_tracker, 7.5, 1.8, &"smooth"))
	wait(0.2)
...
func _update_dot(target: PortObject2D, _delta: float) -> void:
