...
func _ready() -> void:
	...
	play([
		PortMoveTo.new(circle_a, Vector2(980.0, 220.0), 1.5, &"smooth"),
		PortFadeToColor.new(circle_a, Color.WHITE, Color.ORANGE_RED, 1.5, &"smooth"),
	])
	wait_seconds(0.6)
	play([
		PortMoveTo.new(circle_b, Vector2(980.0, 420.0), 1.5, &"smooth"),
		PortFadeToColor.new(circle_b, Color.WHITE, Color.DEEP_SKY_BLUE, 1.5, &"smooth"),
	])
