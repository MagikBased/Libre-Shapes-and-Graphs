class_name LsgObject2D
extends Node2D

var animate: LsgAnimateProxy:
	get:
		return GShapes.AnimateProxy.new(self)
...
func add_updater(updater: Callable) -> void:
	# Updater signature: func updater(target: LsgObject2D, delta: float) -> void
	if updater.is_valid():
