...
func add_member(member: Node3D) -> LsgGroup3D:
	if member == null:
		return self
...
func add_members(new_members: Array) -> LsgGroup3D:
...
func remove_member(member: Node3D) -> LsgGroup3D:
...
func apply_positions(positions: Array[Vector3]) -> LsgGroup3D:
...
func move_group_to(center_position: Vector3) -> LsgGroup3D:
	if members.is_empty():
		return self
