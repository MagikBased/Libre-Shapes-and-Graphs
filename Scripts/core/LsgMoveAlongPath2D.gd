...
func interpolate(alpha: float) -> void:
	...
	var sampled_local: Vector2 = GShapes.PathUtils.sample_polyline(path_points, a, closed_path)
	...
	if orient_to_path:
		...
		var p1: Vector2 = _to_world(GShapes.PathUtils.sample_polyline(path_points, next_alpha, closed_path))
