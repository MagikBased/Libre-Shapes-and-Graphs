...
func recompute_points() -> void:
	...
	for i in range(n):
		...
		var dydx: float = _eval_derivative(x)
		_points.append(axes.c2p(x, dydx))

	queue_redraw()


func eval_derivative(x: float) -> float:
	return _eval_derivative(x)


func _eval_derivative(x: float) -> float:
	var h: float = maxf(0.000001, absf(derivative_epsilon))
