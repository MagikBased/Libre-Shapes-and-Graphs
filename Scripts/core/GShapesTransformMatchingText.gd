class_name GShapesTransformMatchingText
...
var source: GShapesTextMobject
var destination: GShapesTextMobject
...
func _init(
	p_source: GShapesTextMobject,
	p_destination: GShapesTextMobject,
...
func _extract_match_tokens(text_mob: GShapesTextMobject) -> PackedStringArray:
	if text_mob == null:
...
func _estimate_anchor_offset(text_mob: GShapesTextMobject, token: String) -> Vector2:
	if text_mob == null:
		return Vector2.ZERO
...
func _estimate_char_width(text_mob: GShapesTextMobject) -> float:
	return maxf(6.0, float(text_mob.font_size) * 0.52)


func _estimate_line_height(text_mob: GShapesTextMobject) -> float:
...
func _pick_best_common_token(src_mob: GShapesTextMobject, dst_mob: GShapesTextMobject) -> String:
