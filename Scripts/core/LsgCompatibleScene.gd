...
func move_camera(
	...
	play(GShapes.MoveTo.new(camera, to_world_position, run_time, rate_func_name))
...
func zoom_camera(
	...
	var camera := _ensure_scene_camera()
	play(GShapes.CameraZoomTo.new(camera, to_zoom, run_time, rate_func_name))
...
func zoom_to_fit(
	...
	play([
		GShapes.MoveTo.new(_ensure_scene_camera(), center, run_time, rate_func_name),
		GShapes.CameraZoomTo.new(_ensure_scene_camera(), Vector2.ONE * zoom_factor, run_time, rate_func_name),
	])
...
func _ensure_scene_camera() -> Camera2D:
	...
	scene_camera.name = "LsgSceneCamera2D"
