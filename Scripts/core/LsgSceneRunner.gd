...
var active_animations: Array[LsgAnimation] = []
var queued_groups: Array[Array] = []
var paused: bool = false
var _was_busy_last_frame: bool = false


func play(animation: LsgAnimation) -> LsgAnimation:
	if animation == null:
		return null
	animation.begin()
...
func play_many(animations: Array[LsgAnimation]) -> void:
	for animation in animations:
		play(animation)


func queue(animation: LsgAnimation) -> void:
	if animation == null:
		return
	queue_many([animation])


func queue_many(animations: Array[LsgAnimation]) -> void:
...
func wait_seconds(duration: float) -> void:
	queue(GShapes.Wait.new(duration))
