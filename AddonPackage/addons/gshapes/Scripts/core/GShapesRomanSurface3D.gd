class_name GShapesRomanSurface3D
...
var u_min: float = 0.0
var u_max: float = TAU
var v_min: float = -PI * 0.5
var v_max: float = PI * 0.5
var scale_factor: float = 2.4
var phase: float = 0.0
var surface_color: Color = Color(0.46, 0.9, 1.0, 0.88)
...
func _sample(uu: float, vv: float) -> Vector3:
	...
	if mode == "tight":
		...
		ripple = 0.2 * sin(4.0 * u + phase * 1.2) * cos(2.0 * v)

	var cu: float = cos(u)
	var su: float = sin(u)
	var cv: float = cos(v2)
	var sv2: float = sin(2.0 * v2)

	var x: float = sin(2.0 * u) * cv * cv
	var y: float = su * sv2
	var z: float = cu * sv2
	var radial: float = 1.0 + ripple

	return Vector3(x * radial, y, z * radial) * maxf(0.01, scale_factor)
