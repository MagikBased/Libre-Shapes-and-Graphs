...
func add_member(member: Node2D) -> GShapesGroup2D:
	if member == null:
		return self
...
func add_members(new_members: Array) -> GShapesGroup2D:
...
func remove_member(member: Node2D) -> GShapesGroup2D:
...
func clear_members() -> GShapesGroup2D:
	members.clear()
	return self
...
func apply_positions(positions: Array[Vector2]) -> GShapesGroup2D:
...
func move_group_to(center_position: Vector2) -> GShapesGroup2D:
	if members.is_empty():
		return self
...
func set_group_color(group_color: Color) -> GShapesGroup2D:
	for member in members:
		if member is GShapesObject2D:
			(member as GShapesObject2D).color = group_color
		else:
