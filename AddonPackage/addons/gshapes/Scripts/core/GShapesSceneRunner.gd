...
var active_animations: Array[GShapesAnimation] = []
var queued_groups: Array[Array] = []
var paused: bool = false
var _was_busy_last_frame: bool = false


func play(animation: GShapesAnimation) -> GShapesAnimation:
	if animation == null:
		return null
	animation.begin()
...
func play_many(animations: Array[GShapesAnimation]) -> void:
	for animation in animations:
		play(animation)


func queue(animation: GShapesAnimation) -> void:
	if animation == null:
		return
	queue_many([animation])


func queue_many(animations: Array[GShapesAnimation]) -> void:
...
func wait_seconds(duration: float) -> void:
	queue(GShapesWait.new(duration))
