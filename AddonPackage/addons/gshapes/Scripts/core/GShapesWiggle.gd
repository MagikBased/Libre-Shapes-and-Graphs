...
func interpolate(alpha: float) -> void:
	...
	var envelope: float = GShapesRateFunctions.there_and_back(t)
	var phase: float = sin(TAU * wiggles * t)
	target.rotation = _start_rotation + rotation_amplitude * envelope * phase
	var s: float = 1.0 + scale_amplitude * envelope * absf(phase)
