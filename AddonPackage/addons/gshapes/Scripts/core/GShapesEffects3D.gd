...
static func pulse_scale(
	...
	if host == null:
		push_warning("GShapesEffects3D.pulse_scale called without host node")
...
static func spin_y(
	...
	if host == null:
		push_warning("GShapesEffects3D.spin_y called without host node")
		var tree_null := Engine.get_main_loop() as SceneTree
		return tree_null.create_tween()
	if target == null:
		var wait_tween_null_target := host.create_tween()
...
static func arc_move_to(
	...
	if host == null:
		push_warning("GShapesEffects3D.arc_move_to called without host node")
		var tree_null := Engine.get_main_loop() as SceneTree
		return tree_null.create_tween()
	if target == null:
		var wait_tween_null_target := host.create_tween()
...
static func staggered_pulse(
	...
	if host == null:
		push_warning("GShapesEffects3D.staggered_pulse called without host node")
		var tree_null := Engine.get_main_loop() as SceneTree
		return tree_null.create_tween()
